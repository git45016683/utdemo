 ...
 * @LastEditTime: 2022-03-25 02:10:36
...
using ::testing::SetArgPointee;  // for function's output param via pointer
using ::testing::SetArrayArgument;  // for function's output param via array
...
{
TEST_F(modelTest, DISABLED_showData_requestUrlFail) {
    EXPECT_FALSE(pm->showData());
}

TEST_F(modelTest, DISABLED_showData_requestUrlFail_sendSuccess_recvFailed) {
...
TEST_F(modelTest, DISABLED_showData_requestUrlSuccess_sendSuccess_recvSuccess) {
    MockSimulateSocket mock;
    EXPECT_CALL(mock, send(_)).Times(1).WillOnce(Return(32));
...
TEST_F(modelTest, DISABLED_showData_requestUrlFail_sendSuccess_nocallMockrecv) {
    MockSimulateSocket mock;
    EXPECT_CALL(mock, send(_)).Times(1).WillOnce(Return(32));
    pm->setNetwork((network*)&mock);
    EXPECT_TRUE(pm->showData());
...
#pragma region "UT_hardwardResponse"\
{
TEST_F(modelTest, hardwareResponse_Lenis0) {
    auto mockerDevice = MOCKER(&device::receivedatafromdevice);
    EXPECT_CALL(*mockerDevice, MOCK_FUNCTION(_,_)).Times(1).WillOnce(Return(0));
    pm->hardwardResponse();
...
TEST_F(modelTest, hardwareResponse_Error) {
    unsigned char expectValues[1024];
    for (int i = 0; i < 255; i++) expectValues[i] = i;
